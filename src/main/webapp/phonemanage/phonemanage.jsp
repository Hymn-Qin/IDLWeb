...
<body style=" background:#eef2fb;">
	<form action="searchPhoneStatusBySN.action" method="post" id="form1">
		<table width=100% border="0" cellspacing=0 cellpadding=0 width="100%">
			<tr height=30>

				<td colspan="6" background=./img/title_bg1.jpg>當前位置>手机追踪</td>
			</tr>
			<tr>
				<td bgcolor=#b1ceef height=1 colspan="5"></td>
			</tr>
			<tr height=28>
				<td align="left" background=./img/title_bg1.jpg>&nbsp;&nbsp;SN&nbsp;:&nbsp;<input
					type="text" id="sn" name="phoneStatusInfo.sn"></td>
		...
		<%
			...
			if (listPhone != null && listPhone.size() > 0) {
				...
				while (itor.hasNext()) {
					...
					out.println("<td>&nbsp;" + borrow_user + "</td>");
					out.println("<td>&nbsp;" + borrow_user_id + "</td>");
					out.println("<td>&nbsp;" + cabine_id + "</td>");
					out.println("<td>&nbsp;" + box_id + "</td>");
					out.println("<td>&nbsp;" + fault_type + "</td>");
					i++;
				}
			}
		%>
...
var fristpage=1;
$("#lastPage").attr("href","showPhoneStatus.action?pageInfo.nextPage="+pagetotal+"");
$("#nextPage").attr("href","showPhoneStatus.action?pageInfo.nextPage="+pagenext+"");
$("#prePage").attr("href","showPhoneStatus.action?pageInfo.nextPage="+pagebefore+"");
$("#firstPage").attr("href","showPhoneStatus.action?pageInfo.pageNum="+fristpage+"");
	  });
 </script>
</html>
